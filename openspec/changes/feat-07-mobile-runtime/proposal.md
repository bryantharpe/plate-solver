...
packaging. This change specifies `ps-mobile`. Grounded in the performance notes of
`reference-solutions/docs/04, 06, 08` and the PRD's non-functional requirements.

## What Changes

- Introduce the `ps-mobile` crate and the `mobile-runtime` capability: UniFFI bindings (Rust ↔
...
- New crate `ps-mobile` depending on `ps-detect`, `ps-solve`, `ps-db` (and optionally `ps-grpc`),

...
fn prepare(
    ...
    let fov_initial_deg = options
        .fov_estimate
        .unwrap_or_else(|| ((props.min_fov + props.max_fov) / 2.0) as f64);
    let fov_initial = fov_initial_deg.to_radians();

    let num_patterns = (props.num_patterns.max(1)) as usize;
    ...
    let sep_px = width
        * separation_for_density(fov_initial_deg, props.verification_stars_per_fov as f64)
        / fov_initial_deg;
...
mod tests {
    ...
    fn empty_db() -> PatternDatabase {
        PatternDatabase {
            ...
            properties: pattern_database::DatabaseProperties::default(),
        }
    }

//! HTTP web API for the plate solver — a simple harness for testing and
//! experimenting with the solver from a browser.
...
#[derive(Clone)]
pub struct AppState {
    pub db: Arc<PatternDatabase>,
    /// Serializes the solve endpoint: only one decode+solve at a time.
    pub solve_gate: Arc<Semaphore>,
}

impl AppState {
    pub fn new(db: Arc<PatternDatabase>) -> Self {
        Self {
            db,
...
/// git; rebuilding it requires node (`cd crates/ps-web/frontend && npm run
/// build`) but cargo never does — see crates/ps-web/README.md.
#[derive(RustEmbed)]
...
mod tests {
    ...
    use pattern_database::{DatabaseProperties, StarKdTree};
    use tower::ServiceExt;

    fn make_empty_db() -> PatternDatabase {
        PatternDatabase {
            star_table: Vec::new(),
            num_stars: 0,
    ...
    async fn healthz_returns_ok_with_db_info() {
        ...
        // DatabaseProperties::default() pins both FOV bounds at 30 degrees.
        assert_eq!(json["min_fov"], 30.0);

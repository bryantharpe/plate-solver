...
//!
//! The error-path tests run against an empty in-memory database and need no
//! external data. `solve_reference_image_returns_match_found` needs the
//! upstream cedar-solve checkout fetched by `scripts/fetch-references.sh`
//! (for the default database and reference image); it skips with a notice
//! when the checkout is absent.

use axum::body::Body;
use axum::http::{Request, StatusCode};
use http_body_util::BodyExt;
use pattern_database::{load_from_path, DatabaseProperties, PatternDatabase, StarKdTree};
...
/// State backed by an empty database: enough for every request-validation
/// path, and for solves that fail before pattern matching (too_few).
...
/// State backed by the real cedar-solve default database, or `None` when the
/// fetched references checkout is absent.
fn reference_state() -> Option<AppState> {
    let npz = references_dir().join("cedar-solve/tetra3/data/default_database.npz");
    if !npz.is_file() {
        return None;
    }
    let db = load_from_path(&npz).expect("load default_database.npz");
    Some(AppState::new(Arc::new(db)))
}
...
fn load_fixture() -> Fixture {
    ...
    let fixture_path = manifest.join("tests/data/reference_solve.json");
...
fn reference_image_bytes() -> Vec<u8> {
    let img_path = references_dir()
        .join("cedar-solve/examples/data/medium_fov/2019-07-29T204726_Alt40_Azi-135_Try1.jpg");
...
async fn solve_reference_image_returns_match_found() {
    let Some(state) = reference_state() else {
        eprintln!(
            "SKIPPED: references/ not found — run scripts/fetch-references.sh to enable the \
             reference solve test"
        );
        return;
    };

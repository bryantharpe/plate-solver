...
```bash
cargo run -p ps-web -- --db references/cedar-solve/tetra3/data/default_database.npz
```
...
`--db` accepts a tetra3-format `.npz` pattern database. The upstream default
database is available after running `scripts/fetch-references.sh` (path used
in the examples below), or generate your own with the `tetra3-gen-db` CLI.
...
```bash
cd crates/ps-web/frontend && npm run build
```
...
React frontend. Run the smoke check manually after frontend changes:

```bash
cargo run --release -p ps-web -- --db references/cedar-solve/tetra3/data/default_database.npz &
...
```bash
curl -F image=@references/cedar-solve/examples/data/medium_fov/2019-07-29T204726_Alt40_Azi-135_Try1.jpg -F fov_estimate=11 http://127.0.0.1:8080/api/solve
```
